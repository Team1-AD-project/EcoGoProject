...
class MockMapRepository : IMapRepository {
    ...
    override suspend fun getRouteByTransportMode(
        ...
        val resolved = if (apiMode == "transit") {
            resolveTransitRoute(origin, destination, startPoint, endPoint, transportMode)
        } else {
            resolveDirectRoute(origin, destination, apiMode, startPoint, endPoint, transportMode)
        }

        // 计算碳排放
        val carbonData = calculateCarbonForMode(resolved.distance, transportMode)
        ...
        Result.success(RouteRecommendData(
            ...
            total_distance = resolved.distance,
            estimated_duration = resolved.duration,
            total_carbon = carbonData.totalCarbon,
            carbon_saved = carbonData.carbonSaved,
            route_segments = emptyList(),
            route_points = resolved.routePoints,
            route_steps = resolved.steps,
            route_alternatives = resolved.alternatives
        ))
    }

    private suspend fun resolveTransitRoute(
        origin: LatLng,
        destination: LatLng,
        startPoint: GeoPoint,
        endPoint: GeoPoint,
        ...
        return resolveDirectRoute(origin, destination, "walking", startPoint, endPoint, transportMode)
    }

    private suspend fun resolveDirectRoute(
        origin: LatLng,
        destination: LatLng,
        apiMode: String,
        startPoint: GeoPoint,
        ...
        val result = DirectionsService.getRoute(origin, destination, apiMode)
        if (result != null) {
            return ResolvedRoute(
                routePoints = result.points.map { GeoPoint(lng = it.longitude, lat = it.latitude) },
